@foreach ($produtos as $produto)
                    ...
                    <img class="rounded-xl"src="{{ url("storage/{$produto->image}")}}" alt="{{ $produto->title }}" style="max-width:100px">
                       ...
                       <i class="">Preço:R${{ $produto->valor}}</i><br>
                       <i class="">Procedência:{{ $produto->procedencia}}</i><br>
                    <button class="decoration-clone bg-gradient-to-b from-green-200 to-green-500 rounded-xl box-porder p-1"><a href="{{route('produtos.show', $produto->id)}}">Visualizar</a></button>
							@can('is_feirante')                    
                    <button class="decoration-clone bg-gradient-to-b from-green-200 to-green-500 rounded-xl box-porder p-1"><a href="{{route('adicionar_carrinho', ['idproduto' =>$produto->id])}}">Adicionar ao carrinho</a></button>
							@endcan                    
                    @can('is_produtor')
